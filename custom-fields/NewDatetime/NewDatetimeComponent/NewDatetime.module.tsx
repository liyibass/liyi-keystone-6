...
const NewDatetimeWrapper = styled.div`
    @charset 'UTF-8';
    width: 100%;
    display: flex;
    flex-direction: row;
...
export default function NewDatetime(props) {
    const {
        value,
        onChange,
        autoFocus,
    ...
    const nowHandler = (e) => {
        ...
        onChange(nowDate.toISOString())
    }

    const dateFormat = () => {
        return hasTimePicker ? 'yyyy/MM/dd h:mm aa' : 'yyyy/MM/dd '
    }
    ...
    return (
        <NewDatetimeWrapper style={{ opacity: isReadOnly ? 0.6 : 1 }}>
            <DatePicker
                ...
                placeholderText={isReadOnly ? '' : '請輸入日期'}
                readOnly={isReadOnly}
